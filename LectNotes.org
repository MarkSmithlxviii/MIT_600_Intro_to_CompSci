...
scalars are non divisible / atom
 ...
 - does not have char (character) however strings can be of length 1
...
*** Types of programs
 - a straight line program runs from start to finish in a straight line
...
Looping programs execution times are dependant on the size of the input data
rather than the size of the program.
...
* useful commands
raw_input (2.x) and input (3.x) requires input to continue running the script. 
